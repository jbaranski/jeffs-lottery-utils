...
class Lottery:
    ...
    def __post_init__(self):
        try:
            ...
            self.last_entry = ""
            logging.info(
                f"Unable to read last entry because {self.output_absolute_path} does not exist"
            )
    ...
    def wait_and_click(self, xpath: str) -> None:
        el = WebDriverWait(self.chrome_driver, 2).until(
    ...
    def analysis(self):
        ...
        def format_analysis_items(items):
            ...
            for k, v in items:
                if isinstance(k, tuple):
                    ...
                    key_str = str(k)
                arr.append({"type": key_str, "pct": f'{format(v * 100, ".2f")}%'})
            return arr
        ...
        df["even_odd_consecutive"] = df[["even_odd", "consecutive"]].apply(
            tuple, axis=1
        )
        df["lo_hi_consecutive"] = df[["lo_hi", "consecutive"]].apply(tuple, axis=1)
        df["even_odd_lo_hi_consecutive"] = df[
            ["even_odd", "lo_hi", "consecutive"]
        ].apply(tuple, axis=1)
        ...
        even_odd_lo_hi_prob = Pmf.from_seq(df["even_odd_lo_hi"]).sort_values(
            ascending=False
        )
        even_odd_consecutive_prob = Pmf.from_seq(
            df["even_odd_consecutive"]
        ).sort_values(ascending=False)
        lo_hi_consecutive_prob = Pmf.from_seq(df["lo_hi_consecutive"]).sort_values(
            ascending=False
        )
        even_odd_lo_hi_consecutive_prob = Pmf.from_seq(
            df["even_odd_lo_hi_consecutive"]
        ).sort_values(ascending=False)
        ...
        stats = {
            ...
            "white_balls": {
                ...
                "even_odd_consecutive": format_analysis_items(
                    even_odd_consecutive_prob.items()
                ),
                "lo_hi_consecutive": format_analysis_items(
                    lo_hi_consecutive_prob.items()
                ),
                "even_odd_lo_hi_consecutive": format_analysis_items(
                    even_odd_lo_hi_consecutive_prob.items()
                ),
            },
        }
        ...
        if "yellow_ball" in df.columns:
            yellow_ball_prob = Pmf.from_seq(df["yellow_ball"]).sort_values(
                ascending=False
            )
            stats["yellow_ball_hotness"] = format_analysis_items(
                yellow_ball_prob.items()
            )

        with open(
            self.output_absolute_path.replace(".csv", "-analysis.json"), "w"
        ) as f:
    ...
    def chrome_driver_factory() -> WebDriver:
        options = {
            "dev": ["--window-size=1200,1200", "--ignore-certificate-errors"],
            "prod": [
                "--headless",
                "--disable-gpu",
...
class MegaMillions(Lottery):
    ...
    def get_latest_number(self) -> None:
        ...
        time.sleep(2)
        previous_draw = self.chrome_driver.find_elements(
            By.XPATH, '//a[@class="prevDrawItem"]'
        )
    ...
    def extract_one_draw(link: WebElement) -> str:
        draw_date = link.find_elements(By.XPATH, './h5[@class="drawItemDate"]')[
            0
        ].text.strip()
        draw_numbers = [
            x.text.strip()
            for x in link.find_elements(By.XPATH, './ul[@class="numbers"]')[
                0
            ].find_elements(By.XPATH, './li[contains(@class, "ball")]')
        ]
        draw_white_balls = "|".join(draw_numbers[:-1])
        draw_yellow_ball = "".join(draw_numbers[-1:])
        draw_megaplier = (
            link.find_elements(By.XPATH, './span[@class="megaplier pastNumMP"]')[0]
            .text.strip()
            .upper()
        )
...
class Powerball(Lottery):
    ...
    def get_latest_number(self) -> None:
        ...
        if len(previous_draw_arr) == 0:
            ...
            raise Exception(
                "Unable to find previous draw information on Powerball page"
            )
        previous_draw = previous_draw_arr[0]
        draw_date = previous_draw.find_all("h5", {"class": "card-title"})[0].get_text(
            strip=True
        )
        # Example input: Wed, Nov 27, 2024
        draw_date_parsed = datetime.strptime(draw_date, "%a, %b %d, %Y").strftime(
            "%m/%d/%Y"
        )
        draw_numbers = [
            x.get_text(strip=True)
            for x in previous_draw.find_all("div", {"class": "item-powerball"})
        ]
        draw_white_balls = "|".join(draw_numbers[:-1])
        draw_red_ball = "".join(draw_numbers[-1:])
        draw_power_play = (
            previous_draw.find_all("span", {"class": "multiplier"})[0]
            .get_text(strip=True)
            .upper()
        )
        entry = (
            f"{draw_date_parsed},{draw_white_balls},{draw_red_ball},{draw_power_play}\n"
        )
        if entry.strip() != self.last_entry:
            with open(self.output_absolute_path, "a") as f:
                f.write(entry)
    ...
    def get_historical_numbers(self) -> None:
        ...
        items = []
        for link in links:
            draw_date = link.find_elements(By.XPATH, './/h5[@class="card-title"]')[
                0
            ].text.strip()
            # Example input: Wed, Nov 27, 2024
            draw_date_parsed = datetime.strptime(draw_date, "%a, %b %d, %Y").strftime(
            ...
            draw_numbers = [
                x.text.strip()
                for x in link.find_elements(
                    By.XPATH, './/div[contains(@class, "item-powerball")]'
                )
            ]
            draw_white_balls = "|".join(draw_numbers[:-1])
            draw_red_ball = "".join(draw_numbers[-1:])
            draw_power_play = link.find_elements(
                By.XPATH, './/span[@class="multiplier"]'
            )
            ...
            draw_power_play = (
                "0X"
                if len(draw_power_play) == 0
                else draw_power_play[0].text.strip().upper()
            )
            items.append(
                f"{draw_date_parsed},{draw_white_balls},{draw_red_ball},{draw_power_play}\n"
            )
...
def megamillions():
    m = MegaMillions(
        f'{os.getenv("GITHUB_WORKSPACE")}/numbers/megamillions.csv', 70, 25
    )
